...
class FavouriteMoviesRepositoryImpl : FavouriteMoviesRepository {
    ...
    override suspend fun addToFavourites(context: Context, id: String) {
        api.addMovieToFavourites(token = "Bearer " + Repositories.authRepository.getUserToken(context = context).token, id = id)
    }
